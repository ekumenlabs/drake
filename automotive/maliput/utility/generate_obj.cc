...
// within one @p tolerance distance, in meters, away from it.
...
bool DoMeshVerticesLieOnPlane(
  ...
  return std::all_of(
      first, last, [&mesh, &plane, tolerance](const IndexFace::Vertex& vertex) {
        ...
        return (plane.absDistance(x) < tolerance);
      });
}
...
// all @p face vertices lie within a two planes, one @p tolerance
// distance apart from each other. One of them is returned as @p plane.
// @pre Given @p face belongs to the @p mesh.
// @warning If any of the preconditions is not met, this function
//          will abort execution.
bool IsMeshFacePlanar(const GeoMesh& mesh, const IndexFace& face,
  ...
  const Vector3<double>& x1 = GetMeshFaceVertexPosition(mesh, face_vertices[1]);
  const Vector3<double>& x2 = GetMeshFaceVertexPosition(mesh, face_vertices[2]);
  *plane =  Hyperplane3<double>::Through(x0, x1, x2);
  return DoMeshVerticesLieOnPlane(mesh, face_vertices.begin() + 3,
                                  face_vertices.end(), *plane,
                                  tolerance);
}
...
// @param face_index Index of the face to start aggregation with.
...
std::set<int> AggregateAdjacentCoplanarMeshFaces(
  ...
  while (!faces_indices_to_visit.empty()) {
    ...
    const IndexFace& face = faces[face_index];
    Hyperplane3<double> face_plane;
    if (IsMeshFacePlanar(mesh, face, tolerance, &face_plane)) {
      ...
      for (int edge_index = 0; edge_index < face_edge_count; ++edge_index) {
        ...
        if (visited_faces_indices->count(adjacent_face_index) > 0) continue;
        if (IsMeshFaceCoplanarWithPlane(mesh, faces[adjacent_face_index],
                                        face_plane, tolerance)) {
      ...
      features.min_grid_resolution);
  {
    ...
    if (features.simplify_mesh_faces) {
      asphalt_mesh->AddFacesFrom(SimplifyMeshFaces(
          driveable_mesh, base_grid_unit / 20.));
    } else {
  ...
  if (features.draw_elevation_bounds) {
    ...
    if (features.simplify_mesh_faces) {
      h_bounds_mesh->AddFacesFrom(SimplifyMeshFaces(
          upper_h_bounds_mesh, base_grid_unit / 20.));
      h_bounds_mesh->AddFacesFrom(SimplifyMeshFaces(
          lower_h_bounds_mesh, base_grid_unit / 20.));
    } else {

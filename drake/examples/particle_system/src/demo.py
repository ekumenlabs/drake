...
"""Launch the `particle_system` demo simulations with the following supporting
applications:
...
_epilog = """
All remaining arguments are passed to the underlying program:
---
"""
...
class Launcher(object):
    ...
    def wait(self, duration=float('+Inf')):
...
def main():
    workspace_path = "drake/examples/particle_system"
    visualizer_path = "external/drake_visualizer/drake-visualizer"
#    lcm_spy_path = "drake/automotive/lcm-spy"
#    lcm_logger_path = "external/lcm/lcm-logger"

    parser = argparse.ArgumentParser(
        add_help=False, description=__doc__, epilog=_epilog,
        formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("target", type=str, help="demo target to be run")
    parser.add_argument("--dry-run", action='store_true', default=False,
                        help="print commands instead of running them")
    parser.add_argument("--no-visualizer", action='store_true', default=False,
                        help="if given, don't spin up a visualizer")
    args, tail = parser.parse_known_args()

    demo_path = os.path.join(workspace_path, args.target)
    ...
    try:
        if not args.no_visualizer:
            the_launcher.launch([visualizer_path])

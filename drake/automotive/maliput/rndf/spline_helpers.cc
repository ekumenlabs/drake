    ...
    const int _num_of_segments)    
{
  ...
  DRAKE_THROW_UNLESS(_num_segments > 0);
...
{
  for (int i = 0 ; i < this->s_t_.size() ; ++i) {
    ...
    const int _mth, const double _s) const
{
  ...
  // Return t(s) at _s.
  return t_0 + (this->dt_ / ds) * (_s - this->s_t_[i]);
}

ArcLengthParameterizedSpline::ArcLengthParameterizedSpline(
    const std::unique_ptr<Spline>& _spline, const int _num_of_segments)
    : q_t_(std::move(_spline)), t_s(_num_of_segments)
{

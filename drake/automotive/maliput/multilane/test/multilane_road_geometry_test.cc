...
GTEST_TEST(MultilaneLanesTest, DoToRoadPosition) {
  ...
  const auto& lane0 =
      rb->Connect("lane0", 1, 0., 0., 0., kRoadOrigin,
                  ArcOffset(kArcRadius, -kArcDeltaTheta), kFlatZ);
  const auto& lane1 =
      rb->Connect("lane1", 1, 0., 0., 0., lane0->end(), kLength, kFlatZ);
  const auto& lane2 =
      rb->Connect("lane2", 1, 0., 0., 0., lane1->end(),
                  ArcOffset(kArcRadius, kArcDeltaTheta), kFlatZ);
  rb->Connect("lane3a", 1, 0., 0., 0., lane2->end(), kLength, kFlatZ);
  rb->Connect("lane3b", 1, 0., 0., 0., lane2->end(),
              ArcOffset(kArcRadius, kArcDeltaTheta), kFlatZ);
  rb->Connect("lane3c", 1, 0., 0., 0., lane2->end(),
              ArcOffset(kArcRadius, -kArcDeltaTheta), kFlatZ);
...
GTEST_TEST(MultilaneLanesTest, HintWithDisconnectedLanes) {
  ...
  // Define the lanes and connections.
  rb->Connect("lane0", 1, 0., 0., 0., kRoadOrigin0, ArcOffset(50., -M_PI / 2.),
              kFlatZ);
  rb->Connect("lane1", 1, 0., 0., 0., kRoadOrigin1, ArcOffset(50., M_PI / 2.),
              kFlatZ);

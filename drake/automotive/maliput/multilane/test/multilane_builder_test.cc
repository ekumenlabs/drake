...
GTEST_TEST(MultilaneBuilderTest, QuadRing) {
  ...
  auto left1 = b.Connect("left1", 1, 0., kLeftShoulder, kRightShoulder,
                         northbound.reverse(), kLargeClockwiseLoop, kFlatZ);
  // This heads +y, loops to -x, counterclockwise, back to origin.
  auto left0 = b.Connect("left0", 1, 0., kLeftShoulder, kRightShoulder,
                         northbound, kSmallCounterClockwiseLoop, kFlatZ);
  // This heads +y, loops to +x, clockwise, back to origin.
  auto right0 = b.Connect("right0", 1, 0., kLeftShoulder, kRightShoulder,
                          northbound, kSmallClockwiseLoop, kFlatZ);
  // This heads -y, loops to +x, counterclockwise, back to origin.
  auto right1 =
      b.Connect("right1", 1, 0., kLeftShoulder, kRightShoulder,
  ...
  b.SetDefaultBranch(left1, 0, api::LaneEnd::kStart, left1, 0,
                     api::LaneEnd::kFinish);
  b.SetDefaultBranch(left0, 0, api::LaneEnd::kFinish, right0, 0,
                     api::LaneEnd::kStart);
  b.SetDefaultBranch(right0, 0, api::LaneEnd::kFinish, right1, 0,
                     api::LaneEnd::kFinish);
  b.SetDefaultBranch(right1, 0, api::LaneEnd::kStart, left1, 0,
                     api::LaneEnd::kFinish);

  b.SetDefaultBranch(left1, 0, api::LaneEnd::kFinish, left1, 0,
                     api::LaneEnd::kStart);
  b.SetDefaultBranch(left0, 0, api::LaneEnd::kStart, right1, 0,
                     api::LaneEnd::kStart);
  b.SetDefaultBranch(right0, 0, api::LaneEnd::kStart, right1, 0,
                     api::LaneEnd::kStart);
...
class MultilaneBuilderPrimitivesTest : public ::testing::Test {
 protected:
  ...
  const EndpointZ kLowFlatZ{0., 0., 0., 0.};
  const Endpoint start{{0., 0., -M_PI / 4.}, kLowFlatZ};
};
...
TEST_F(MultilaneBuilderPrimitivesTest, MultilaneLineSegment) {
  ...
  b.Connect("c0", 3, kR0, kLeftShoulder, kRightShoulder, start, kLength,
            kLowFlatZ);
  std::unique_ptr<const api::RoadGeometry> rg =
  ...
  for (int i = 0; i < kNumLanes; i++) {
    EXPECT_EQ(rg->junction(0)->segment(0)->lane(i)->id().string(),
              std::string("l:c0_") + std::to_string(i));
  }
  EXPECT_EQ(rg->num_branch_points(), 6);
  for (int i = 0; i < 6; i++) {
    const api::BranchPoint* bp = rg->branch_point(i);
    EXPECT_EQ(bp->id().string(), std::string("bp:") + std::to_string(i));
    const api::Lane* lane = rg->junction(0)->segment(0)->lane(i / 2);
    if (i % 2 == 0) {
      EXPECT_EQ(bp->GetASide()->size(), 1);
      EXPECT_EQ(bp->GetASide()->get(0).lane, lane);
      EXPECT_EQ(bp->GetBSide()->size(), 0);
    } else {
      EXPECT_EQ(bp->GetASide()->size(), 1);
      EXPECT_EQ(bp->GetASide()->get(0).lane, lane);
      EXPECT_EQ(bp->GetBSide()->size(), 0);
...
TEST_F(MultilaneBuilderPrimitivesTest, MultilaneArcSegment) {
  ...
  b.Connect("c0", 3, kR0, kLeftShoulder, kRightShoulder, start,

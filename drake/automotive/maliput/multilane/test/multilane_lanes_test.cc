...
TEST_P(MultilaneLanesParamTest, FlatLineLane) {
  ...
  Segment* s1 = rg.NewJunction(api::JunctionId{"j1"})
                    ->NewSegment(api::SegmentId{"s1"}, std::move(road_curve_1),
                                 1, r0, 0., -kHalfWidth + r0, kHalfWidth + r0,
                                 {0., kMaxHeight});
  ...
  Segment* s2 = rg.NewJunction(api::JunctionId{"j2"})
                    ->NewSegment(api::SegmentId{"s2"}, std::move(road_curve_2),
                                 1, r0, 0., -kHalfWidth + r0, kHalfWidth + r0,
                                 {0., kMaxHeight});
  Lane* l1_with_z = s2->NewLane(api::LaneId{"l1_with_z"},
...
TEST_P(MultilaneLanesParamTest, FlatArcLane) {
  ...
  Segment* s1 = rg.NewJunction(api::JunctionId{"j1"})
                    ->NewSegment(api::SegmentId{"s1"}, std::move(road_curve_1),
                                 ...
                                 {0., kMaxHeight});
  Lane* l2 =
      s1->NewLane(api::LaneId{"l2"}, {-kHalfLaneWidth, kHalfLaneWidth});
  ...
  Segment* s2 = rg.NewJunction(api::JunctionId{"j2"})
                    ->NewSegment(api::SegmentId{"s2"}, std::move(road_curve_2),
                                 1, r0, 0., -kHalfWidth + r0, kHalfWidth + r0,
                                 {0., kMaxHeight});
  Lane* l2_with_z = s2->NewLane(api::LaneId{"l2_with_z"},
                                {-kHalfLaneWidth, kHalfLaneWidth});
  EXPECT_TRUE(api::test::IsLanePositionClose(
...
GTEST_TEST(MultilaneLanesTest, ArcLaneWithConstantSuperelevation) {
  ...
  Segment* s1 =
      ...
      ->NewSegment(api::SegmentId{"s1"}, std::move(road_curve_1), kNumLanes,
                   kR0, kRSpacing, -kHalfWidth + kR0, kHalfWidth + kR0,
                   {0., kMaxHeight});
...
TEST_F(MultilaneMultipleLanesTest, MultipleLineLanes) {
  ...
  Segment* s1 = rg.NewJunction(api::JunctionId{"j1"})
                    ->NewSegment(api::SegmentId{"s1"}, std::move(road_curve),
                                 kNumLanes, kR0, kRSpacing, kRMin, kRMax,
                                 height_bounds);
  Lane* l0 = s1->NewLane(api::LaneId{"l0"}, {-8., kHalfLaneWidth});
  Lane* l1 = s1->NewLane(api::LaneId{"l1"}, {-kHalfLaneWidth, kHalfLaneWidth});
  Lane* l2 = s1->NewLane(api::LaneId{"l2"}, {-kHalfLaneWidth, 2.});
  const std::vector<Lane*> lanes{l0, l1, l2};
  ...
  for (const Lane* lane : lanes) {
    for (const double p : p_vector) {
      for (const double r : r_offset_vector) {
        ...
        EXPECT_TRUE(api::test::IsGeoPositionClose(
            nearest_position, geo_point, kVeryExact));
        EXPECT_NEAR(distance, 0., kVeryExact);
      }
    }
    lane_spacing += kRSpacing;
...
TEST_F(MultilaneMultipleLanesTest, MultipleArcLanes) {
  ...
  // Checks length of the three Lanes.
  const std::vector<double> lane_radius{
    (kRadius - kR0),
      ...
      for (const double r : r_offset_vector) {
        ...
        EXPECT_TRUE(api::test::IsRotationClose(
            ...
            api::Rotation::FromRpy(
                0., 0., wrap_angle(effective_angle + (0.5 * M_PI))),
            kVeryExact));
      }
    }
  }
  ...
  for (size_t i = 0; i < lanes.size(); i++) {
    ...
    for (const double r : r_offset_vector) {
      EXPECT_TRUE(api::test::IsLanePositionClose(
          ...
          api::LanePosition((radius / (radius - r)) * 1., 1., 1.),
          kVeryExact));
      EXPECT_TRUE(api::test::IsLanePositionClose(
          lane->EvalMotionDerivatives({lane->length(), r, 100.}, {1., 1., 1.}),
          api::LanePosition(radius / (radius - r), 1., 1.),
          kVeryExact));
    }
  }
}

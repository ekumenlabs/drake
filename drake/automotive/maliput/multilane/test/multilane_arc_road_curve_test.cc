...
// - No offset.
// - Zero offset radius.
// - Negative offset radius.
// - Bigger offset radius.
// - Smaller offset radius.
GTEST_TEST(MultilaneArcRoadCurve, IsValidTest) {
  ...
  const ArcRoadCurve flat_arc_geometry(kZeroCenter, kRadius, kInitTheta,
                                       kDTheta1, zp, zp);
  // Checks with good lateral bounds.
  EXPECT_TRUE(flat_arc_geometry.IsValid(lateral_bounds, height_bounds, 0.));
  EXPECT_FALSE(
      flat_arc_geometry.IsValid(lateral_bounds, height_bounds, kRadius));
  EXPECT_FALSE(
      flat_arc_geometry.IsValid(lateral_bounds, height_bounds, 2.0 * kRadius));
  EXPECT_TRUE(
      flat_arc_geometry.IsValid(lateral_bounds, height_bounds, -kRadius));
  EXPECT_TRUE(flat_arc_geometry.IsValid(lateral_bounds, height_bounds,
                                        -0.25 * kRadius));
  // Checks with large_lateral bounds.
  EXPECT_FALSE(
      flat_arc_geometry.IsValid(large_lateral_bounds, height_bounds, 0.));
  EXPECT_FALSE(
      flat_arc_geometry.IsValid(large_lateral_bounds, height_bounds, kRadius));
  ...
  const double cos_superelevation = std::cos(M_PI / 4.0);
  // Checks with good lateral bounds.
  EXPECT_TRUE(
      right_handed_cone_geometry.IsValid(lateral_bounds, height_bounds, 0.));
  EXPECT_FALSE(right_handed_cone_geometry.IsValid(
      lateral_bounds, height_bounds, kRadius * cos_superelevation));
  EXPECT_FALSE(right_handed_cone_geometry.IsValid(
      lateral_bounds, height_bounds, 2.0 * kRadius * cos_superelevation));
  EXPECT_TRUE(right_handed_cone_geometry.IsValid(
      lateral_bounds, height_bounds, -kRadius * cos_superelevation));
  EXPECT_TRUE(right_handed_cone_geometry.IsValid(
      lateral_bounds, height_bounds, -0.25 * kRadius * cos_superelevation));
  // Checks with large lateral bounds.
  EXPECT_FALSE(right_handed_cone_geometry.IsValid(large_lateral_bounds,
                                                  height_bounds, 0.));
  EXPECT_FALSE(right_handed_cone_geometry.IsValid(
      large_lateral_bounds, height_bounds, kRadius * cos_superelevation));
  EXPECT_FALSE(right_handed_cone_geometry.IsValid(
      large_lateral_bounds, height_bounds, 2.0 * kRadius * cos_superelevation));
  EXPECT_TRUE(right_handed_cone_geometry.IsValid(
      large_lateral_bounds, height_bounds, -kRadius * cos_superelevation));
  EXPECT_TRUE(
      right_handed_cone_geometry.IsValid(large_lateral_bounds, height_bounds,
                                         -0.25 * kRadius * cos_superelevation));
  // Checks with critical lateral bounds.
  EXPECT_FALSE(right_handed_cone_geometry.IsValid(critical_cone_lateral_bounds,
                                                  height_bounds, 0.));
  EXPECT_FALSE(right_handed_cone_geometry.IsValid(
      critical_cone_lateral_bounds, height_bounds,
      kRadius * cos_superelevation));
  EXPECT_FALSE(right_handed_cone_geometry.IsValid(
      critical_cone_lateral_bounds, height_bounds,
      2.0 * kRadius * cos_superelevation));
  EXPECT_TRUE(right_handed_cone_geometry.IsValid(
      critical_cone_lateral_bounds, height_bounds,
      -kRadius * cos_superelevation));
  EXPECT_TRUE(right_handed_cone_geometry.IsValid(
  ...
  // Checks with good lateral bounds.
  EXPECT_TRUE(
      left_handed_cone_geometry.IsValid(lateral_bounds, height_bounds, 0.));
  EXPECT_FALSE(left_handed_cone_geometry.IsValid(
      lateral_bounds, height_bounds, -kRadius * cos_superelevation));
  EXPECT_FALSE(left_handed_cone_geometry.IsValid(
      lateral_bounds, height_bounds, -2.0 * kRadius * cos_superelevation));
  ...
  EXPECT_TRUE(left_handed_cone_geometry.IsValid(
      lateral_bounds, height_bounds, -0.25 * kRadius * cos_superelevation));
  // Checks with large lateral bounds.
  EXPECT_FALSE(left_handed_cone_geometry.IsValid(large_lateral_bounds,
                                                 height_bounds, 0.));
  EXPECT_FALSE(left_handed_cone_geometry.IsValid(
      large_lateral_bounds, height_bounds, -kRadius * cos_superelevation));
  EXPECT_FALSE(
  ...
  EXPECT_FALSE(left_handed_cone_geometry.IsValid(
      critical_cone_lateral_bounds, height_bounds,
      -2.0 * kRadius * cos_superelevation));
  EXPECT_TRUE(left_handed_cone_geometry.IsValid(critical_cone_lateral_bounds,
                                                height_bounds,
                                                kRadius * cos_superelevation));
  EXPECT_TRUE(left_handed_cone_geometry.IsValid(
      critical_cone_lateral_bounds, height_bounds,

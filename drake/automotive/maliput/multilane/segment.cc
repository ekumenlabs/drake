...
Segment::Segment(const api::SegmentId& id, api::Junction* junction,
  ...
  const double last_lane_centerline = r0 +
      static_cast<double>(num_lanes_ - 1) * std::copysign(r_spacing_, r0_);
...
Lane* Segment::NewLane(api::LaneId id, const api::RBounds& lane_bounds) {
  ...
  auto lane_ = std::make_unique<Lane>(id, this, index, lane_bounds,
                                      driveable_bounds, elevation_bounds_,
                                      road_curve_.get(), r_offset);

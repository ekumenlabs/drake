...
class IntegralFunction {
 public:
  ...
  T operator()(T b, const VectorX<T>& params) const {
    return this->operator()(static_cast<T>(0.0), b, params);
  }
  ...
  /// given on construction.
  /// @warning This method will abort if preconditions are not met.
  T operator()(T a, T b, const VectorX<T>& params) const;

...
///
///  - Junctions: "j:" + Connection::id() or "j:" + Group::id()
///  - Segments: "s:" + Connection::id()
///  - Lanes: "l:" + Connection::id() + "_" + lane_index
///  - BranchPoints: "bp:" + branch_point_index
///
/// Note: when Connections are ungrouped and one Junction each Segment is
/// created, Connection's ID is preferred.
///
...
/// bearing multiple Lanes with offsets of the reference path. The
...
class Connection {
 public:
  ...
  ///
  /// @param num_lanes The number of lanes. It must be grater than zero.
  /// @param r0 Distance from the reference curve to the first Lane centerline.
  /// @param left_shoulder The extra space at the right side of the first lane
  /// that adds to Segment's bounds. It must be grater or equal to zero.
  /// @param right_shoulder The extra space at the left side of the last lane
  /// that adds to Segment's bounds. It must be grater or equal to zero.
  ...
  /// @param num_lanes The number of lanes. It must be grater than zero.
  /// @param r0 Distance from the reference curve to the first Lane centerline.
  /// @param left_shoulder The extra space at the left side of the last lane
  /// that adds to Segment's bounds. It must be grater or equal to zero.
  /// @param right_shoulder The extra space at the right side of the first lane
  /// that adds to Segment's bounds. It must be grater or equal to zero.
  Connection(const std::string& id, const Endpoint& start, const Endpoint& end,
             int num_lanes, double r0, double left_shoulder,
...
class Builder {
 public:
  ...
  ///
  /// `r_spacing` is the distance that separates consecutive Lanes in the same
  /// Segment. It must be greater or equal to zero. Segment extents will be
  /// derived from the composition of left and right shoulders, number of lanes
  /// and spacing between Lanes. In addition, `r_spacing` will become the width
  /// of all the Lanes, and lane bounds will be assigned over the centerline.
  /// The `elevation_bounds` is applied uniformly to all lanes of every segment.
  /// `linear_tolerance` and `angular_tolerance` specify the respective
  /// tolerances for the resulting RoadGeometry.
  Builder(double r_spacing, const api::HBounds& elevation_bounds,

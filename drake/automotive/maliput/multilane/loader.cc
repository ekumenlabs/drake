...
std::unique_ptr<const api::RoadGeometry> BuildFrom(YAML::Node node) {
  ...
  const api::RBounds driveable_bounds(r_bounds(mmb["lane_bounds"]));
  const double left_shoulder = std::abs(driveable_bounds.min()) -
                               std::abs(lane_bounds.min());
  const double right_shoulder = std::abs(driveable_bounds.max()) -
                                std::abs(lane_bounds.max());

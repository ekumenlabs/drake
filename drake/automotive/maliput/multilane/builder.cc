...
Builder::Builder(double r_spacing, const api::HBounds& elevation_bounds,
    ...
    : r_spacing_(r_spacing), elevation_bounds_(elevation_bounds),
...
const Connection* Builder::Connect(const std::string& id, int num_lanes,
  ...
  connections_.push_back(
      std::make_unique<Connection>(id, start, end, num_lanes, r0, left_shoulder,
                                   right_shoulder));
...
const Connection* Builder::Connect(const std::string& id, int num_lanes,
  ...
  connections_.push_back(
      std::make_unique<Connection>(id, start, end, num_lanes, r0, left_shoulder,
          right_shoulder, cx, cy, arc.radius(), arc.d_theta()));
  return connections_.back().get();
}
...
std::vector<Lane*> Builder::BuildConnection(
  ...
  const double r_max = conn->r0() +
                       r_spacing_ * (static_cast<double>(conn->num_lanes()) +
                                     0.5) +
                       conn->left_shoulder();
  ...
  for (int i = 0; i < conn->num_lanes(); i++) {
    Lane* lane = segment->NewLane(api::LaneId{std::string("l:") + conn->id() +
                                              std::string("_") +
                                              std::to_string(i)},
                                  conn->r0() +
                                      r_spacing_ * static_cast<double>(i),

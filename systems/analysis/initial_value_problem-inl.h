...
template <typename T>
std::unique_ptr<ContinuousExtension<T>> InitialValueProblem<T>::DenseSolve(
    const T& tf, const SpecifiedValues& values) const {
  // Gets all values to solve with, either given or default, while
  // checking that all preconditions hold.
  const SpecifiedValues safe_values = SanitizeValuesOrThrow(tf, values);
  ...
  // Starts dense integration to build a continuous extension.
  std::unique_ptr<ContinuousExtension<T>> continuous_extension =
  ...
  // the built continuous extension.
  integrator_->StopDenseIntegration();

  // Yields continuous extension to the caller.
